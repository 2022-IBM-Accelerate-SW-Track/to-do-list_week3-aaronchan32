...
test("test that App component doesn't render dupicate Task", () => {
  ...
  fireEvent.change(inputDate, { target: { value: dueDate } });
  fireEvent.click(element);
  const check = screen.getAllByText('History Test');
...
test('test that App component renders different colors for past due events', () => {
  ...
  const inputDate = screen.getByPlaceholderText('mm/dd/yyyy');
  const element = screen.getByRole('button', { name: /Add/i });
  const pastDueDate = '10/30/1999';
  fireEvent.change(inputTask, { target: { value: 'Overdue Task' } });
  fireEvent.change(inputDate, { target: { value: pastDueDate } });
  fireEvent.click(element);
  const futureDueDate = '10/30/2023';
  fireEvent.change(inputTask, { target: { value: 'Future Task' } });
  fireEvent.change(inputDate, { target: { value: futureDueDate } });
  fireEvent.click(element);
  const overdueColor = screen.getByTestId('Overdue Task').style.backgroundColor;
  const futureColor = screen.getByTestId('Future Task').style.backgroundColor;
  console.log(overdueColor);
  console.log(futureColor);
  expect(overdueColor).not.toBe(futureColor);
});
